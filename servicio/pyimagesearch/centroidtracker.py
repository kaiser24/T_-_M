...
class CentroidTracker:
	def __init__(self, maxDisappeared=50, maxDistance=30, vehiclesTypes=None, SAVE_DETS = False, SAVE_PATH = None):
		...
		self.carTypeObjects = OrderedDict()
	...
	def carsRouteout(self):
		return self.carRouteOut
		
	def carsTypes(self):
		return self.carTypeObjects
	def boxes(self):
		return self.boxes

	def register(self, centroid,carType, box, frame):
		
		#print("Entró: ", self.nextObjectID, centroid)
		self.carRouteIn[self.nextObjectID] = centroid
		...
		self.carTypeObjects[self.nextObjectID] = carType
	...
	def deregister(self, objectID):
		...
		#print("Salió: ", objectID, self.objects[objectID])
		self.carRouteOut[objectID] = self.objects[objectID]
		...
		del self.carTypeObjects[objectID]
		del self.disappeared[objectID]

	def update(self, rects,carTypesList, frame):
		...
		if len(rects) == 0:
			...
			# to update
			return self.objects, self.carTypeObjects, self.boxes
		...
		if len(self.objects) == 0:
			for i in range(0, len(inputCentroids)):
				self.register(inputCentroids[i],carTypesList[i], rects[i], frame)
		...
		# centroids
			...
			for (row, col) in zip(rows, cols):
				...
				#if D[row, col] > radius( self.objects[objectID][1] ):
				#	continue
				...
				#self.carTypeObjects[objectID] = carTypesList[col]
			...
			# register each new input centroid as a trackable object
				...
				for col in unusedCols:
					self.register(inputCentroids[col],carTypesList[col],rects[col], frame)
